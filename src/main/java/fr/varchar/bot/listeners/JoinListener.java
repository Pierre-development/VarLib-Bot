...
public class JoinListener extends ListenerAdapter
{
    ...
    {
        event.getGuild().addRoleToMember(event.getMember().getId(), event.getGuild().getRoleById("848579159567368193")).queue();
        if(!event.getUser().isBot()) {
            ...
            event.getGuild().getTextChannelById("835137363809533972").sendMessage(embedBuilder.build()).queue();
        }
    }
}
